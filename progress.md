...
### Step 25: Generalized Task Execution Pipeline & Action State Isolation
* *Completed*:
  1. **Generic Semantic Target Resolver** ([`packages/privacy-core/src/semantic-target-resolver.js`](packages/privacy-core/src/semantic-target-resolver.js)): Implemented multi-signal scoring algorithm evaluating `labelText`, `ariaLabel`, `placeholder`, `name`, `id`, `text`/`value`, and semantic types (`mode: "input"` vs `mode: "click"`). No website-specific selectors or hardcoded IDs.
  2. **Bidirectional Goal Parsing & Click Extraction** ([`packages/privacy-core/src/goal-parser.js`](packages/privacy-core/src/goal-parser.js)): Generalized NLP patterns to support `change X to Y`, `enter Y into X`, `update X to Y`, `type Y in X`, `replace X with Y`, and `set X to Y`. Extracted explicit compound click targets (`confirm order`, `submit`, `continue`, `cancel`) and prevented click phrases from falsely flagging the task as `INSPECT`.
  3. **Strict Task Planning & Action State Isolation** ([`packages/privacy-core/src/task-planner.js`](packages/privacy-core/src/task-planner.js)): Ensured `fill_form` is only planned when field updates or form filling are instructed; planned click actions strictly with `actionType: "CLICK"` and `value: null`; strictly preserved multi-action execution order (`fill_form` -> `submit_form` -> `verify_goal`).
  4. **Action Binding & Fallback Isolation in Runtime and Server** ([`apps/extension/src/popup.js`](apps/extension/src/popup.js), [`scripts/extension-log-server.mjs`](scripts/extension-log-server.mjs)): Gated search strictly to search tasks (preventing search fallbacks from hijacking OTP/text fields); resolved form fields top-to-bottom in DOM order using `ActiveResolveSemanticTarget`; removed automatic fall-through to `submit_form`; and isolated `CLICK` action parameters so they never inherit `value` or `text` from preceding `FILL` actions.
  5. **Controlled Demo Portal Dynamic Feedback** ([`tests/fixtures/controlled-privacy-demo.html`](tests/fixtures/controlled-privacy-demo.html)): Implemented generic action execution feedback toast and dynamic confirmation view with zero hardcoded confirmation strings.
  6. **Comprehensive Automated Test Suite** ([`tests/generalized-semantic-pipeline.test.mjs`](tests/generalized-semantic-pipeline.test.mjs)): 11 comprehensive automated tests covering tasks A through K (independent sequential tasks, zero value/target bleeding, unseen fields/buttons, NLP variations, and source-level hardcoding audits).
* *Evidence*: All 582 tests across 15 test suites pass (`582 pass, 0 fail`); Chrome extension builds cleanly via [`scripts/build-extension.mjs`](scripts/build-extension.mjs); all 355 core privacy unit tests pass.
